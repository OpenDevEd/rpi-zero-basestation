...
def write_to_log(text, type):
    try:
        db.db_data_event_create("Data Logging", "Success", type, text, "sensorboard")
    except Exception as e:
        print("Error writing to log: " + str(e))
        db.db_data_event_create("Data Logging", "Failure", type, str(e), "sensorboard")


def write_data(text):
    try:
        db.db_data_log_create("sensorboard", text, "string")
        db.db_data_event_create(
            "Data Logging", "Success", "Sensor Reading", "sensor reading", "sensorboard"
        )
    except Exception as e:
        print("Error writing to log: " + str(e))
        db.db_data_event_create(
...
if settings["scd4x"]:
    ...
    write_to_log(
        "scd4x: Serial number:" + str([hex(i) for i in scd4x.serial_number]), "init"
    )
    scd4x.start_periodic_measurement()
    write_to_log(
        "scd4x: Waiting for first measurement....", "waiting first measurement"
    )

if settings["ahtx0"]:
    write_to_log("ahtx0: init", "init")
    ahtx0 = adafruit_ahtx0.AHTx0(i2c)

if settings["bh1750"]:
    write_to_log("bh1750: init", "init")
...
try:
    while True:
        if settings["ahtx0"]:
            write_data("ahtx0: Temperature (*C)=%0.2f" % ahtx0.temperature)
            write_data("ahtx0: Humidity (%%)=%0.2f" % ahtx0.relative_humidity)
        if settings["bh1750"]:
            write_data("bh1750: Light (lux)=%0.2f" % bh1750.lux)
        if settings["scd4x"] and scd4x.data_ready:
            write_data("scd4x: CO2=%d ppm" % scd4x.CO2)
            write_data("scd4x: Temperature (*C)=%0.2f" % scd4x.temperature)
            write_data("scd4x: Humidity (%%)=%0.2f" % scd4x.relative_humidity)
        if settings["pm"]:
            data = pms5003.read()
            write_data("pm: " + str(pmdata(data)))
        ...
        if settings["ens160"]:
            ...
            write_data("ens160: AQI (1-5)=" + str(ens.AQI))
            write_data("ens160: TVOC (ppb)=" + str(ens.TVOC))
            write_data("ens160: eCO2 (ppm)=" + str(ens.eCO2))
